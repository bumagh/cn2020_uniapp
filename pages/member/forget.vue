  ...
  export default {
    ...
    methods: {
      ...
      getcode() {
		...
		let that = this;
		let pdata = {};
		pdata.phone = this.phone;
		utils.post(getApp().globalData.eyUrl+'tools/getPhoneCode',pdata).then(res=>{
			let ret = res.data;
			if (ret.code == '1'){
				_this.second = 60;
		...
		/*
        uni.request({
          url: 'getcode.html', //仅为示例，并非真实接口地址。
          data: {
            phone: this.phone,
            type: 'forget'
          },
      ...
      bindLogin() {
		...
		utils.post(getApp().globalData.eyUrl+'member/setPwdByPhone',pdata).then(res=>{
			let ret = res.data;
			if (ret.code == '1'){
				uni.showToast({
					title: '密码成功重置',
